...
const selectedIcludesElementID = (id: string) => props.selected.includes(id)
const anyElementSelected = computed(() => {
  return elements.value.some(el => selectedIcludesElementID(el.id))
})
...
<template>
  <div 
    class="highlight" 
    :class="{visible: anyElementSelected}"
  />
  <div class="range">
    <h3>Base</h3>
...
.highlight {
  ...
  outline: solid 4px var(--foreground);

  transition: all 0.2s ease-in-out, opacity 0.01s;
}
